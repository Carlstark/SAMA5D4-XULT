...
static int ads7846_get_irq_dt(struct device *dev)
{
    int ret;
    struct spi_device *spi = to_spi_device(dev);

    ret = of_get_named_gpio(dev->of_node, "irq", 0);
    if (ret > 0) {
        spi->irq = gpio_to_irq (ret);
    }
...
static int ads7846_get_irq_dt(struct device *dev) { return 0; }
#endif
